...
AP_VideoTX::AP_VideoTX()
{
    ...
    band_names = {'A','b','E','F','r','P','L','U','O','X'};
    _max_band = 10;
...
uint16_t AP_VideoTX::get_table_frequency_mhz(uint8_t band, uint8_t channel) {
    return VIDEO_CHANNELS[band][channel];
}

bool AP_VideoTX::get_band_and_channel(uint16_t freq, uint8_t& band, uint8_t& channel)
{
    for (uint8_t i = 0; i < _max_band; i++) {
