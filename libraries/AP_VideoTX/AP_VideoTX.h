...
public:
    ...
    std::vector<std::vector<uint16_t>> VIDEO_CHANNELS;

    uint16_t get_table_frequency_mhz(uint8_t band, uint8_t channel);
    bool get_band_and_channel(uint16_t freq, uint8_t& band, uint8_t& channel);
    ...
    uint8_t max_band() const { return _max_band; }
    uint8_t max_channels() const { return VTX_MAX_CHANNELS; }
